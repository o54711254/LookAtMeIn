...
function FaceSymmetry() {
    ...
    const handleImageAnalyze = async () => {
        try {
            const detections = await faceapi.detectSingleFace(imageRef.current, new faceapi.SsdMobilenetv1Options()).withFaceLandmarks();
            const canvas = canvasRef.current;
            faceapi.matchDimensions(canvas, { width: imageRef.current.width, height: imageRef.current.height });

            if (detections) {
                const resizedDetections = faceapi.resizeResults(detections, { width: imageRef.current.width, height: imageRef.current.height });
    ...
    return (
        <div>
            ...
            {image && (
                <>
                    <img ref={imageRef} src={image} alt="Face" onLoad={handleImageAnalyze} />
